...
public class TileGenerator : MonoBehaviour
{
    ...
    public void GenerateTilesByConfig(LevelConfig config)
    {
        ...
        AllBariers = new Barrier[X, Y];
        if(config.ScoreQuest) QuestsManager.SpawnScoreQuest(X * Y * 8);
        if(config.ItemQuest) QuestsManager.SpawnItemQuest(X * Y * 2);
        SearchStartPosition();
        for (int i = 0; i < X; i++)
        {
            for (int j = 0; j < Y; j++)
        ...
        if(config.BarrierQuest) QuestsManager.SpawnBariersQuest();
        if (!MatchManager.CheckStepAvailable()) _tileGenerator.RefillTiles();
    }
    ...
    public void AlignObjects()
    {
        ...
        if(lastTilePos < 0) lastTilePos *= -1;
    ...
    public static void FillAfterMatch()
    {
        ...
        for (int i = 0; i < X; i++)
        {
            for (int j = 0; j < Y; j++)
            {
                ...
                {
                    if (AllBariers[i, j] != null && AllBariers[i, j].Type == Barrier.BarrierType.Ice) continue;
                    if (AllBariers[i, j - 1] != null && AllBariers[i, j - 1].Type == Barrier.BarrierType.Ice) continue;
                    tmpY = j - 1;
                    while (j > 0)
                    {
    ...
    public void RefillTiles()
    {
        ...
        if (!MatchManager.CheckStepAvailable()) RefillTiles();
    }
